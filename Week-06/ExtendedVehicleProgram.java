// Vehicle Program

public class ExtendedVehicleProgram {
    public static void main(String[] args) {

        Geared twoWheeler1 = new Geared();

        // TwoWheeler twoWheeler1 = new TwoWheeler();
        twoWheeler1.vehicleInput(1, 123, "blue");
        twoWheeler1.twoWheelerInput(1000, 2000);
        twoWheeler1.getConsumption(100);
        twoWheeler1.displayConsumption();
        twoWheeler1.maintanance();
        twoWheeler1.average();

        // Geared gearedTwoWheeler1 = new Geared();
        twoWheeler1.gearedInput("Geared", "Tesla");

        twoWheeler1.getType();
        twoWheeler1.getName();

        System.out.println("\n");

        FourWheeler fourWheeler1 = new FourWheeler();
        fourWheeler1.vehicleInput(2, 456, "red");
        fourWheeler1.fourWheelerInput(2000, 3000);
        fourWheeler1.getConsumption(200);
        fourWheeler1.displayConsumption();
        fourWheeler1.maintanance();
        fourWheeler1.average();
    }
}

class Vehicle {
    int vehicleNumber;
    int insuranceNumber;
    String color;
    int fuel;

    void vehicleInput(int vehicleNumber, int insuranceNumber, String color) {
        this.vehicleNumber = vehicleNumber;
        this.insuranceNumber = insuranceNumber;
        this.color = color;
    }

    void getConsumption(int fuel) {
        this.fuel = fuel;
    }

    void displayConsumption() {
        System.out.println("Fuel Consumption: " + fuel);
    }
}

class TwoWheeler extends Vehicle {

    int maintanance;
    int average;

    void twoWheelerInput(int maintanance, int average) {
        this.maintanance = maintanance;
        this.average = average;
    }

    void maintanance() {
        System.out.println("TwoWheeler Maintainance: " + maintanance);
    }

    void average() {
        System.out.println("TwoWheeler Average: " + average);
    }
}

class Geared extends TwoWheeler {

    String type;
    String name;

    void gearedInput(String type, String name) {
        this.type = type;
        this.name = name;
    }

    void getType() {
        System.out.println("TwoWheeler Type: " + type);
    }

    void getName() {
        System.out.println("TwoWheeler Name: " + name);
    }
}

class NonGeared extends TwoWheeler {
    String type;
    String name;

    void nonGearedInput(String type, String name) {
        this.type = type;
        this.name = name;
    }

    void getType() {
        System.out.println("TwoWheeler Type: " + type);
    }

    void getName() {
        System.out.println("TwoWheeler Name: " + name);
    }
}

class FourWheeler extends Vehicle {
    int maintanance;
    int average;

    void fourWheelerInput(int maintanance, int average) {
        this.maintanance = maintanance;
        this.average = average;
    }

    void maintanance() {
        System.out.println("FourWheeler Maintainance: " + maintanance);
    }

    void average() {
        System.out.println("FourWheeler Average: " + average);
    }
}